...
function Body({ selectedPlaylist, tracks, loading, searchTerm, onSearchTermChange }) {
  return (
    <main className="bg-gradient-to-b from-neutral-900 via-neutral-900 to-neutral-950 px-4 py-6 md:px-8 md:py-8">
      ...
      {!selectedPlaylist ? (
        ...
        <>
          <div className="mb-6 flex flex-col gap-4 rounded-2xl border border-white/10 bg-white/5 p-4 shadow-xl md:flex-row md:items-end md:p-6">
            {selectedPlaylist?.images?.[0]?.url ? (
              <img
                src={selectedPlaylist.images[0].url}
                alt={selectedPlaylist.name}
                className="h-32 w-32 rounded-xl object-cover shadow-lg md:h-40 md:w-40"
              />
            ) : (
              <div className="grid h-32 w-32 place-items-center rounded-xl bg-gradient-to-br from-green-600 to-emerald-400 text-4xl text-black shadow-lg md:h-40 md:w-40">
                ♪
              </div>
            )}
            <div>
              <p className="text-xs font-semibold uppercase tracking-[0.18em] text-neutral-400">Playlist</p>
              <h2 className="mt-2 text-3xl font-extrabold tracking-tight md:text-5xl">{selectedPlaylist.name}</h2>
